...
import { SchemaUrlAutocomplete } from "../components/SchemaUrlAutocomplete";
    ...
    <section aria-label="Load schema" className={`schema-loader${loaderOpen ? " is-open" : ""}`} onDragOver={(event) => { if ([...event.dataTransfer.types].includes("Files")) event.preventDefault(); }} onDrop={receiveSchemaFile} onPaste={receiveSchemaFile}>
      {!loaderOpen && primary ? <div className="schema-summary">
        ...
        {programId === "none" ? <div className="custom-schema-controls">
          ...
          {loaderAction === "url" ? <div className="schema-input-reveal schema-url-input"><label htmlFor="schema-url">HTTPS schema URL (.json, .yaml, .yml, .toml)</label><p className="schema-hint">Start typing to pick from the <a href={SCHEMASTORE_SITE_URL} rel="noreferrer" target="_blank">SchemaStore catalog</a>{catalog.length ? ` (${catalog.length} schemas)` : ""}.</p><div><SchemaUrlAutocomplete catalog={catalog} disabled={schemaBusy} id="schema-url" onChange={setSchemaUrl} placeholder="https://example.com/schema.yaml" value={schemaUrl} /><button className="button button-primary" disabled={!schemaUrl.trim() || schemaBusy} onClick={() => void fetchSchema()} type="button">{schemaBusy ? "Fetching..." : "Fetch schema"}</button></div></div> : null}
